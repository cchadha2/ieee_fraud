...
# Maybe a weighted average taking more important M features into account would be better here
...
train['rapid_transaction_and_address_mismatch'] = (train['TransactionDT_check'] < 200) & (train['address_match'] < 0.5)
test['rapid_transaction_and_address_mismatch'] = (test['TransactionDT_check'] < 200) & (test['address_match'] < 0.5)

train['address_match_2'] = country_match(train)
test['address_match_2'] = country_match(test)

train['rapid_transaction_and_address_mismatch_2'] = (train['TransactionDT_check'] < 200) & (train['address_match_2'] == 0)
test['rapid_transaction_and_address_mismatch_2'] = (test['TransactionDT_check'] < 200) & (test['address_match_2'] == 0)
...
X = train.drop(['TransactionID', 'TransactionDT',
            'uid','uid2', 'bank_type',
...
X_test = test.drop(['TransactionID', 'TransactionDT',
            'uid','uid2', 'bank_type',
            'DeviceInfo','DeviceInfo_device',
            'id_30','id_30_device','id_30_version',
            'id_31','id_31_device', 'id_33',
...
sub_df.to_csv('../submissions/submission_cv_0.9303291743305049.csv', index=False)
